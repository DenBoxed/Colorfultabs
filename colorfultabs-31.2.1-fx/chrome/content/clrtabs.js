var colorfulTabs = {
	...
	colorfulTabsUrlListener: {
		...
		onLocationChange: function (aProgress, aRequest, aURI) {
			...
			try {
				...
				var url = tab.linkedBrowser.contentDocument.location.href;
				if (!url || !host) {
					return;
				}
				var pat;
				if (colorfulTabs.enabledomain) {
					...
					if (!tabClr) { // if the domain didn't get a match, (preference - color unmatched domains)generate a default color?
						if (!colorfulTabs.urlbased) {
							...
							tabClr = 'hsl(' + Math.abs(colorfulTabs.clrHash(url)) % 360 + ',' + colorfulTabs.sat + '%,' + colorfulTabs.lum + '%)';
						}
					}
				} else {
					tabClr = 'hsl(' + Math.abs(colorfulTabs.clrHash(host)) % 360 + ',' + colorfulTabs.sat + '%,' + colorfulTabs.lum + '%)';
	...
	colorfulTabsOptions: {
		...
		//resets preferences in Color Fine-Tuning
		resetPref: function () {
			...
			if (scheme == 1) {
				try {
					ctPref.clearUserPref("extensions.clrtabs.satmin");
				} catch (e) {};
				try {
					ctPref.clearUserPref("extensions.clrtabs.satmax");
				} catch (e) {};
				try {
					ctPref.clearUserPref("extensions.clrtabs.lummin");
				} catch (e) {};
				try {
					ctPref.clearUserPref("extensions.clrtabs.lummax");
				} catch (e) {};
			}
			if (scheme == 2) {
				try {
					ctPref.clearUserPref("extensions.clrtabs.sat");
				} catch (e) {};
				try {
					ctPref.clearUserPref("extensions.clrtabs.lum");
				} catch (e) {};
			}
		},
		...
		adv_toggle_state: function (checked, caller) {
			...
			try {
				...
				colorfulTabs.cl('Error in : adv_toggle_state' + e);
			}
			return true;
		},
	...
	clrtabsInit: function () {
		...
		gBrowser.mTabContainer.mAllTabsPopup.addEventListener("popupshowing", colorfulTabs.setMIcolor, false);
	...
	ctFirstRun: function (ctVersion) {
		...
		if (colorfulTabs.ctVersion == '0') {
			clrUrl = 'https://www.addongenie.com/fr/colorfultabs?vi='
		} else {
			clrUrl = 'https://www.addongenie.com/fr/colorfultabs?vu='
		}
	...
	setDomainPref: function () {
		...
		var params = {
			inn: {
				oldColor: 'rgb(' + colorfulTabs.clrSession.getTabValue(clrObj, "tabClr").toString() + ')',
				domain: clrObj.linkedBrowser.contentDocument.location.host,
				enabled: 0
			},
			out: null
		};
	...
	initTabcontext: function () {
		if (Components.classes["@mozilla.org/preferences-service;1"].getService(Components.interfaces.nsIPrefBranch).getBoolPref("extensions.clrtabs.menu").toString() == 'true') {
			...
			if (document.getElementById('colorfulTabsSeparator'))
				...
				colorfulTabs.cl('Error in inittabcontext: ' + e);
				return;
			}
		}
	},
	...
	setCtPref: function () {
		...
		try {
			...
			colorfulTabs.urlbased = clrprefs.getBoolPref("extensions.clrtabs.urlbased");
	...
	handletabdblclick: function(evt) {
	...
	setColor: function (tab, tabClr) {
		...
		if (!colorfulTabs.isMac) {
			...
			tab.style.setProperty('background-image', macClr, colorfulTabs.aggressive);
		}
		try {
		...
		for (var i = 0; i < gBrowser.mTabs.length; i++) {
			...
			if (tab.mCorrespondingMenuitem) {
				if (colorfulTabs.clrAllTabsPopPref) {
					...
					tab.mCorrespondingMenuitem.style.setProperty('background-image', 'none', 'important');
				}
			}
		}
	},
	...
	//a formatted dump()
	cl: function (msg) {
		if (colorfulTabs.ctdebug) console.log("\ncl:\t" + msg);
	},

	setstandout: function () {
		var tabClr;
		try { //remove highlighting from all other tabs
	...
	setTaBottomClr: function () {
		try {
			...
			colorfulTabs.cl("Error in setTaBottomClr: " + e)
		}
	},
...
var colorfulTabsPrefObserver = {
	...
	observe: function (aSubject, aTopic, aData) {
		...
		switch (aData) {
			case "ppref":
				var ppref = prefBranch.getCharPref("ppref");
				ppref = ppref.split('~');
				colorfulTabs.tabColors = ppref;
				break;
			case "txtshadow":
			...
			case "tablabelblink":
				...
				break;
			case "urlbased":
				colorfulTabs.urlbased = prefBranch.getBoolPref("urlbased");
				break;
			case "minify":
